  ...
  .factory("websocket", function(store, $timeout, $ngRedux, $state) {
        ...
        s.socket.onmessage = function(message) {
          ...
          switch(response.type) {
            ...
            case "auth_response":
              ...
              break;
            case "live_data":
              let device = _.find($ngRedux.getState().devices, {"rid":response.payload.rid})
              $ngRedux.dispatch(websocketLiveData(device, response.payload.alias, response.payload.data));
              break;
          }
        }
      }
    ...
    var s = {
      ...
      socketSend: function(payload) {
        let state = $ngRedux.getState();
        if(state.websocket.connected || payload.type === "auth") {
      ...
      subscribe: function(rids, aliases) {
        let s = this;
        if(!(rids instanceof Array)) {
          rids = [rids];
        }
        if(!(aliases instanceof Array) && aliases) {
          aliases = [aliases];
        }
        s.socketSend({type: "subscribe", rids: rids, aliases: aliases})
      },
