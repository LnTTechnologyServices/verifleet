...
module.exports = function(sequelize, DataTypes) {
    var Device = sequelize.define('device', {
        ...
        instanceMethods: {
            ...
            readData: function(aliases_1, options) {
                console.log("Before Reading alias: ", JSON.stringify(options));
                var defer = Q.defer();
                if (!options) {
                    options = { limit: 1 };
                }
                ...
                var infoDefer = new simplep_1.exoDefer(auth);
                if (aliases_1) {
                    infoDefer.info(options).then(function(info) {
                        var dataResult = [];
                        _.forEach(aliases_1, function(alias) {
                            dataResult.push(alias.alias);
                        });
                        var aliases = dataResult;
                        if (aliases_1.length) {
                            var data_1 = {};
                            var readDefer_1 = new simplep_1.exoDefer(auth);
                            _.each(aliases_1, function(alias) {
                                readDefer_1.read(alias.alias, alias.options);
                            });
                            readDefer_1.then(function(results) {
                                _.each(results, function(result) {
                                    var alias = aliases[result.id];
                                    // console.log("Reading alias _ Name: ", alias);
                                    var datapoints = _.map(result.result, function(datapoint) {
                                        ...
                                        try {
                                            ...
                                            // console.log("Reading alias _ Results_Catch: ", JSON.stringify(point));
                                        }
                                        return point;
                                    });
